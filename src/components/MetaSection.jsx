...
export default function MetaSection({ title, badge, defaultOpen = false, rows = [] }) {
  ...
  return (
    <div className="mx-2 mb-1.5 rounded-md border border-border-warm overflow-hidden">
      <Collapsible open={open} onOpenChange={setOpen}>
        <CollapsibleTrigger className="flex w-full items-center justify-between py-2.5 px-3 bg-surface-2 hover:brightness-95 transition-colors duration-fast group">
        ...
        <CollapsibleContent>
          <div className="px-4 pb-3 pt-1 flex flex-col bg-surface-1">
            {rows.map((row) => (
